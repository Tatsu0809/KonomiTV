...
def Installer(version: str) -> None:
    ...
    if is_git_installed is True:
        ...
        result = RunSubprocess(
            ...
            ['git', 'clone', '-b', revision, 'https://github.com/Tatsu0809/KonomiTV.git', install_path.name],
    ...
    # Git コマンドがインストールされていない場合: zip でダウンロード
        ...
        if version == 'latest':
            source_code_response = requests.get('https://codeload.github.com/Tatsu0809/KonomiTV/zip/refs/heads/zero-trust')
        else:
            source_code_response = requests.get(f'https://codeload.github.com/Tatsu0809/KonomiTV/zip/refs/tags/v{version}')
        ...
        if version == 'latest':
            shutil.move(install_path.parent / 'KonomiTV-zero-trust/', install_path)
        else:
        ...
        with progress:
            ...
            subprocess.run(
                args = [
                    ...
                    f'program={install_path /server/.venv/Scripts/python.exe ""}',
                ],
                stdout = subprocess.DEVNULL,  # 標準出力を表示しない
                stderr = subprocess.DEVNULL,  # 標準エラー出力を表示しない
            )
